...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // ── State ──────────────────────────────────────────────────────────────────
  // All Supabase calls now go through the Cloudflare Worker (service role key server-side)
  var EC = {
    worker: 'https://evolved-caveman-proxy.john-834.workers.dev',
    proxy: 'https://evolved-caveman-proxy.john-834.workers.dev',  // Claude API route
  ...
  // Check consent via Worker /check-consent (service role key server-side)
  function ecCheckAccess() {
    ...
    xhr.open('GET', EC.worker + '/check-consent?email=' + encodeURIComponent(email), true);
    xhr.onload = function() {
      if (xhr.status === 200) {
        var data = JSON.parse(xhr.responseText);
        if (data.found) {
          var consentDate = data.record && data.record.timestamp ? new Date(data.record.timestamp) : new Date();
          var daysLeft = EC.accessDays - Math.floor((new Date() - consentDate) / 86400000);
  ...
  // Create session via Worker /session
  function ecInitSession() {
    var xhr = new XMLHttpRequest();
    xhr.open('POST', EC.worker + '/session', true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.onload = function() {
      try { var d = JSON.parse(xhr.responseText); if (d && d.id) EC.sessionId = d.id; } catch(e) {}
    };
  ...
  // Log crisis via Worker /crisis
  function ecLogCrisis(category) {
    var xhr = new XMLHttpRequest();
    xhr.open('POST', EC.worker + '/crisis', true);
    xhr.setRequestHeader('Content-Type', 'application/json');
  ...
  // Update session via Worker PATCH /session
  function ecUpdateSession() {
    if (!EC.sessionId) return;
    var xhr = new XMLHttpRequest();
    xhr.open('PATCH', EC.worker + '/session?id=' + EC.sessionId, true);
    xhr.setRequestHeader('Content-Type', 'application/json');
